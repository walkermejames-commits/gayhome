...
CREATE TABLE partner_organisations (
  id uuid PRIMARY KEY, region_id text NOT NULL REFERENCES regions(id), name text NOT NULL, partner_type text NOT NULL, status text NOT NULL DEFAULT 'invited',
  data_sharing_status text NOT NULL DEFAULT 'none', verification_expires_at timestamptz, suspended_at timestamptz, revoked_at timestamptz, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(),
  CHECK(status IN ('invited','pending_verification','verified','limited','suspended','expired','revoked','archived'))
);
CREATE INDEX partner_organisations_region_status_idx ON partner_organisations(region_id,status);
...
CREATE TABLE partner_users (organisation_id uuid NOT NULL REFERENCES partner_organisations(id) ON DELETE CASCADE, user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE, role text NOT NULL, status text NOT NULL DEFAULT 'pending', created_at timestamptz NOT NULL DEFAULT now(), PRIMARY KEY(organisation_id,user_id), CHECK(role IN ('administrator','service_editor','verifier','viewer')), CHECK(status IN ('pending','active','suspended','revoked')));
CREATE TABLE partner_agreements (id uuid PRIMARY KEY, organisation_id uuid NOT NULL REFERENCES partner_organisations(id) ON DELETE CASCADE, agreement_type text NOT NULL, status text NOT NULL DEFAULT 'draft', effective_at timestamptz, expires_at timestamptz, document_reference text, created_at timestamptz NOT NULL DEFAULT now());
...
CREATE TABLE provider_update_proposals (
  id uuid PRIMARY KEY, organisation_id uuid NOT NULL REFERENCES partner_organisations(id), service_external_id text NOT NULL, field_name text NOT NULL,
  current_value jsonb NOT NULL, proposed_value jsonb NOT NULL, reason text NOT NULL, evidence_metadata jsonb NOT NULL DEFAULT '{}'::jsonb,
  submitted_by uuid NOT NULL REFERENCES users(id), submitted_at timestamptz, urgency text NOT NULL DEFAULT 'standard', reviewer_id uuid REFERENCES users(id),
  decision text, approved_value jsonb, dataset_revision_id uuid REFERENCES regional_dataset_revisions(id), publication_date timestamptz,
  status text NOT NULL DEFAULT 'draft', hidden_at timestamptz, hide_reason text, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(),
  CHECK(status IN ('draft','submitted','under_review','approved_for_revision','rejected','withdrawn','published')),
  CHECK(status <> 'published' OR (reviewer_id IS NOT NULL AND decision='approved' AND dataset_revision_id IS NOT NULL AND publication_date IS NOT NULL))
);
CREATE INDEX provider_proposals_queue_idx ON provider_update_proposals(status,urgency,submitted_at);
