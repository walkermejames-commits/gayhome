...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const urgency = parsed.data.urgency === "safety_critical" ? "critical" : parsed.data.urgency === "urgent" ? "high" : "standard";
    await client.query("INSERT INTO service_correction_proposals(id,record_type,record_external_id,current_value,proposed_value,reason,evidence,urgency,submitted_by,partner_organisation_id,verification_status) VALUES($1,'service',$2,$3,$4,$5,$6,$7,$8,$9,'pending')", [id, parsed.data.serviceId, { [parsed.data.field]: parsed.data.currentValue }, { [parsed.data.field]: parsed.data.proposedValue }, parsed.data.reason, [{ source: parsed.data.evidenceReference ?? "partner_attestation", bytesAccepted: false }], urgency, access.userId, access.organisationId]);
    await client.query("INSERT INTO audit_events(id,actor_id,event_type,resource_type,resource_id,metadata) VALUES($1,$2,'provider.proposal_submitted','service_correction',$3,$4)", [randomUUID(), access.userId, id, { organisationId: access.organisationId, field: parsed.data.field }]);
